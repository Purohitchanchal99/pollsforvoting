...
def polls_list(request):
    ...
    paginator = Paginator(all_polls, 8)  # Show 6 contacts per page
...
def add_choice(request, poll_id):
    ...
    if request.user != poll.owner:
        return redirect('home')
    #code for checking vote
    if poll.choice_set.count() > 0:
        messages.warning(
            request, "Votes have already been submitted. You cannot add more choices.", extra_tags='alert alert-warning alert-dismissible fade show')
        return redirect('pollapp:edit', poll.id)
...
def user_view(request):
    ...
    return render(request, 'polls/user_poll.html', context)